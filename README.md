# issizim
İŞSİZİM - Sansürsüz Sığınak
"Kasıntı vitrinlerden, sahte başarı hikayelerinden ve iş dünyasının bitmek bilmeyen tutarsızlıklarından sıkıldık."

Nedir?
İş arama süreçlerinde, mülakatlarda ve ofis içi mobbinglerde yaşananların; kurumsal maskelerden arındırılarak sansürsüzce paylaşıldığı bir sığınak.

Sürümlerimiz:
Projeyi geliştirmeye devam ediyoruz. Farklı arayüz deneyimleri için iki versiyonumuz mevcut:

v.1 (Masaüstü Odaklı)

v.2 (Mobil Uyumlu & Dashboard)

Vizyonumuz:
Bu iki versiyon, profesyonel bir platforma dönüşme yolculuğumuzun ilk adımlarıdır. Amacım, teknik ve tasarımsal detayları bu vizyona inanan paydaşlarla birlikte hayata geçirip kalıcı bir topluluk platformu inşa etmektir.

İletişim & Katkı:
Bu sadece bir kod yığını değil, ortak Sığınağımız. Deneyimlerini paylaşmak veya projeye katkıda bulunmak istersen LinkedIn üzerinden bana ulaşabilirsin.

Not: Yapay zeka ile birlikte geliştirdik.
Farklı versiyonlar denedim v.1 https://issizim.rf.gd/
                           v.2 https://t-han-9.github.io/issizim/

https://www.linkedin.com/in/thanozbek/
e-posta: tunahan.1776@outlook.com
